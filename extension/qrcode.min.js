var qrcode = (function () {
    ...
    QRCode.prototype = {
        ...
        _makeImpl: function (test, maskPattern) {
            ...
            for (var row = 0; row < this._moduleCount; row++) {
                this._modules[row] = new Array(this._moduleCount);
                for (var col = 0; col < this._moduleCount; col++) {
            ...
            if (this._dataCache == null) {
                this._dataCache = QRCode.createData(this._typeNumber, this._errorCorrectLevel, this._dataList);
            }
        ...
        createTableTag: function (cellSize, margin) {
            ...
            for (var r = 0; r < this.getModuleCount(); r++) {
                ...
                for (var c = 0; c < this.getModuleCount(); c++) {
                    ...
                    qrHtml += ' background-color: ' + (this.isDark(r, c) ? '#000000' : '#ffffff') + ';';
                    qrHtml += '"/>';
                }
                qrHtml += '</tr>';
            }
        ...
        createSvgTag: function (cellSize, margin) {
            ...
            var c, mc, draw, r, c, v;
            var opts = { cellSize: cellSize, margin: margin };
            if (typeof arguments[0] === 'object') {
                opts = arguments[0];
                cellSize = opts.cellSize || 2;
                margin = (typeof opts.margin == 'undefined') ? cellSize * 4 : opts.margin;
            }
            size = this.getModuleCount() * cellSize + margin * 2;
            ...
            for (r = 0; r < this.getModuleCount(); r++) {
                for (c = 0; c < this.getModuleCount(); c++) {
                    if (this.isDark(r, c)) {
                        ...
                        svg += '<rect x="' + x + '" y="' + y + '" width="' + rectSize + '" height="' + rectSize + '"/>';
                    }
                }
            }
            svg += '</g></svg>';
        ...
        createImgTag: function (cellSize, margin) {
            ...
            var min = margin;
            var max = size - margin;
            return '<img src="' + this.createDataURL(cellSize, margin) + '" width="' + size + '" height="' + size + '"/>';
        },
    ...
    QRRSBlock.RS_BLOCK_TABLE = [
        [1, 26, 19], [1, 26, 16], [1, 26, 13], [1, 26, 9], [1, 44, 34], [1, 44, 28], [1, 44, 22], [1, 44, 16], [1, 70, 55], [1, 70, 44], [1, 70, 34], [1, 70, 26], [1, 100, 80], [1, 100, 64], [1, 100, 48], [1, 100, 36], [1, 134, 108], [1, 134, 86], [1, 134, 62], [1, 134, 46], [2, 86, 68], [2, 86, 56], [2, 86, 40], [2, 86, 30], [2, 98, 78], [2, 98, 64], [2, 98, 48], [2, 98, 36], [2, 120, 96], [2, 120, 78], [2, 120, 58], [2, 120, 44], [2, 132, 106], [2, 132, 84], [2, 132, 64], [2, 132, 50], [4, 144, 116], [4, 144, 90], [4, 144, 70], [4, 144, 52], [2, 86, 68, 2, 86, 69], [4, 69, 43, 1, 70, 44], [6, 43, 19, 2, 44, 20], [8, 37, 13, 4, 38, 14], [4, 81, 67, 1, 82, 68], [4, 43, 24